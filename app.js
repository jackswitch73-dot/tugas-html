...
function setScene({ enemy = false, npc = "", shop = false } = {}) {
  ...
  if (enemy) {
    ...
    el.enemySprite.src = `assets/${sprite}`;
    el.enemySprite.alt = "Enemy sprite";
  }
  if (shop) {
    el.npcSprite.src = "assets/shopkeeper.png";
    el.npcSprite.alt = "Shopkeeper sprite";
  } else if (npc) {
    el.npcSprite.src = `assets/${npc}`;
...
function openShop(which, withIntro = true) {
  ...
  setScene({ shop: true });
  const items = [
    ["hp", "assets/hp-potion.png", "Restore +20 HP"],
    ["mp", "assets/mp-potion.png", "Restore +20 MP"],
    ["poison", "assets/poison-potion.png", "Damage musuh -50% untuk serangan berikutnya"],
    ["frost", "assets/frost-potion.png", "Musuh tidak menyerang 1 turn"],
  ];
